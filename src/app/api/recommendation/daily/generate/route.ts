...
// 从 Prisma Work 推断标签（接收 genres/themes 为 string[]）
type WorkLike = {
  genres: string[];
  themes: string[];
  continent: string;
...
function computeTags(w: WorkLike): string[] {
  const tags: string[] = [];
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!prefs || (prefs as string[]).length === 0) {
      return NextResponse.json({ error: "未设置偏好" }, { status: 400 });
    }
    const prefList = prefs as string[];
    ...
    if (existing) {
      const book = await prisma.work.findUnique({ where: { id: existing.bookId } });
      return NextResponse.json({
        recommendation: { bookId: existing.bookId, date: existing.date, reason: existing.reason, book },
        cached: true,
      });
    }
    ...
    const scored = allWorks
      ...
      .map((w) => {
        const workLike: WorkLike = {
          genres: (w.genres as string[]) ?? [],
        ...
        const tags = computeTags(workLike);
        const matches = tags.filter((t) => prefList.includes(t));
      ...
      try {
        const resp = await fetch("https://api.anthropic.com/v1/messages", {
          ...
          body: JSON.stringify({
            ...
            messages: [{
              role: "user",
              content: `你是一个文学推荐助手。用户偏好标签：${prefList.join("、")}。推荐的书籍是《${best.work.title}》（${best.work.author}，${best.work.country}），匹配的标签：${best.tags.filter((t: string) => prefList.includes(t)).join("、")}。请用中文写一段2-3句的个性化推荐理由，说明为什么根据用户的偏好推荐这本书。直接输出推荐理由，不需要书名或作者介绍。`
            }],
          }),
    ...
    if (!reason) {
      const matchedTags = best.tags.filter((t) => prefList.includes(t));
    ...
    const rec = await prisma.dailyRecommendation.create({
      data: { userId: session.user.id, bookId: best.work.id, date: today, reason },
    });

    return NextResponse.json({
      recommendation: { bookId: rec.bookId, date: rec.date, reason: rec.reason, book: best.work },
      cached: false,
    });
  } catch (err) {
